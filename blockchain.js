...
class Block {
    ...
    calculateHash(){
        return SHA256(this.calculateProperty(this.index) + this.calculateProperty(this.name) + 
        this.calculateProperty(this.email) + this.calculateProperty(this.polling_booth) + this.calculateProperty(this.admin_name) + 
        this.calculateProperty(this.previous_hash)).toString();
    }
    // isValid()
}


const Genesis = new Block (0,"Genesis Block","genesis@email.com",99,"Genesis");
