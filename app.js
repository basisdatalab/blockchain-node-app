...
app.post("/voter", async (req, res) => {
  try {
    ...
    if (genesis.rows == 0) {
      let genesisBlock = new Block(0,"genesis", "email@Genesis.com",0,"genAdmin", "iniceritanyahashpanjanggitu");
      const genesisInsert = await pool.query(
        ...
        //   genesisBlock.calculateProperty(genesisBlock.admin_name),genesisBlock.calculateProperty(genesisBlock.previous_hash)]
          [genesisBlock.index,genesisBlock.name,genesisBlock.email,genesisBlock.polling_booth,genesisBlock.admin_name,genesisBlock.previous_hash]
    ...
    // create new hash
      ...
      [Voter.index,Voter.name,Voter.email,Voter.polling_booth, Voter.admin_name,Voter.previous_hash]
